...
export default class MainScreen extends React.Component {
  ...
  getRSS(_isLoader) {
    ...
    getNews(global.lang).then((rss) => {
      ...
      this.setState({ news: rss.items, firstLoader: true }, () => {
  ...
  render() {
    ...
    var myNewsList = this.state.myCategorys.map((item, index) => {
      return (
        <UiNewsCard
          key={index}
          selectedTheme={global.theme}
          image={parseTags(item.description).images}
          title={item.title} category={item.categories[0].name}
        ...
        <SafeAreaView style={styles.safeArea} forceInset={{ top: 'never' }}>
          {this.state.offline && !this.state.loader ?
            <View style={styles.noContent}>
              <UiScreenMsg
                selectedTheme={global.theme}
                noTitle={Locale(global.lang, "connect_off_news")}
                noText={Locale(global.lang, "connect_off_news_text")}
                noButtonText={Locale(global.lang, "choice_of_subject")}
              />
            </View> : null
          }

          {!this.state.offline && this.state.myCategorys.length == 0 && !this.state.isSearch ?
            <View style={styles.noContent}>
              <UiScreenMsg
                selectedTheme={global.theme}
                noTitle={Locale(global.lang, "no_title")}
                noText={Locale(global.lang, "choice_of_subject_for_you")}
                noButton
                noButtonText={Locale(global.lang, "choice_of_subject")}
                onPress={() => this.props.navigation.navigate("NewsCategory")}
              />
            </View>
            : null}

   
          {this.state.myCategorys.length > 0 && !this.state.isSearch ?
            <ScrollView style={styles.content}>

              {this.state.myCategorys.length == 0 ?
                <UiScreenMsg
                  selectedTheme={global.theme}
                  noTitle={Locale(global.lang, "no_title2")}
                  noButton
                  noButtonText={Locale(global.lang, "choice_of_subject")}
                  onPress={() => this.props.navigation.navigate("NewsCategory")}
                />
